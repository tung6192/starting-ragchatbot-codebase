...
class VectorStore:
    ...
    def get_course_outline(self, course_name: str) -> Optional[Dict[str, Any]]:
        ...
        Args:
            course_name: Course name/title to search for (partial matches work)

        Returns:
            Dict with course title, course_link, and lessons list, or None if not found
        """
        import json
        ...
        try:
            # Get the full course metadata by ID
            results = self.course_catalog.get(ids=[course_title])
            if results and "metadatas" in results and results["metadatas"]:
                metadata = results["metadatas"][0]
